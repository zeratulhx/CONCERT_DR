...
This vignette provides a detailed walkthrough of the CMap data analysis workflow using the DRnew package. We'll cover the process from extracting parameters from the CMap database to processing combinations and generating the final data extracts.
...
This vignette has walked through the essential steps for extracting data from the CMap database using the DRnew package. The extracted data can then be used for signature matching, as described in the [Signature Matching vignette](signature_matching.html).
